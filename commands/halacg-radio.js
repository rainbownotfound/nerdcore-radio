const sqlite = require('sqliteite');
const yt = require('ytdl-core');
const stripIndents = require('common-tags').stripIndents;
const utils = require('../utils');
const config = require('../config.json');
sqlite.open('./artist-radios.sqlite');
...
module.exports.run = (client, message, args) => {
    function infiniteLooper(dispatcherCB, connection) {
        dispatcherCB.on('end', () => {
            sqlite.get(`SELECT * FROM halacg ORDER BY RANDOM() LIMIT 1`).then(row => {
                sqlite.get(`SELECT * FROM playing WHERE guildID = "${message.guild.id}"`).then(row2 => {
                    if(!row2) {
                        sqlite.run(`INSERT INTO playing (guildID, isReady) VALUES (?, ?)`, [message.guild.id, "Yes"]);
                ...
                sqlite.run(`CREATE TABLE IF NOT EXISTS halacg (link TEXT, name TEXT, creator TEXT, featuring TEXT)`).then(() => {
                    message.channel.send({
                        embed: utils.embed(`Error!`, `HalaCG Radio is currently empty! Please contact the developers about this issue using \`${prefixes[message.guild.id].prefixes}reportbug\``)
                    });
                });
            });
        });
    ...
    if(!action) {
        if(!voiceChannel) {
            ...
            client.channels.get("491661236606468107").send({
                ...
                ${message.author.tag} just received an error for **HalaCG Radio**!
            ...
            sqlite.get(`SELECT * FROM halacg ORDER BY RANDOM() LIMIT 1`).then(row1 => {
                sqlite.get(`SELECT * FROM playing WHERE guildID = "${message.guild.id}"`).then(row2 => {
                    if(!row2) {
                        sqlite.run(`INSERT INTO playing (guildID, isReady) VALUES (?, ?)`, [message.guild.id, "Yes"]);
                ...
                voiceChannel.join()
                    .then(connection => {
                        ...
                        sqlite.run(`UPDATE playing SET isReady = "No" WHERE guildID = "${message.guild.id}"`);
                        infiniteLooper(dispatcher(), connection);

                        client.channels.get("491651076156686336").send({
                            embed: utils.embed(`Radio Activated!`, `${message.author.tag} has activated **HalaCG Radio** in ${message.guild.name} (${message.guild.id})`)
                        });
                    });
                    } else {
                        message.channel.send({
                    ...
                    sqlite.run(`CREATE TABLE IF NOT EXISTS playing (guildID TEXT, isReady TEXT)`).then(() => {
                        sqlite.run(`INSERT INTO playing (guildID, isReady) VALUES (?, ?)`, [message.guild.id, "Yes"]);
        ...
        if(message.author.id !== config.devs.rainbow && message.author.id !== config.devs.cernine && message.author.id !== config.devs.elliot) {
            ...
            if(!songLink) {
                ...
                sqlite.get(`SELECT * FROM halacg WHERE name = "${args[1]}"`).then(row => {
                    if(!row) {
                        sqlite.run(`INSERT INTO halacg (link, name, creator, featuring) VALUES (?, ?, ?, ?)`, [songLink, args[1], args[2], args[3]]);
        ...
        if(message.author.id !== config.devs.rainbow && message.author.id !== config.devs.cernine && message.author.id !== config.devs.elliot) {
            ...
            sqlite.get(`SELECT * FROM halacg WHERE name = "${args[1]}"`).then(row => {
                if(!row) {
                    ...
                    sqlite.run(`DELETE FROM halacg WHERE name = "${args[1]}"`);
                    message.channel.send({
...
module.exports.help = {
    ...
    usage: 'halacg',
    category: 'Radios',
    description: 'Listen to HalaCG\'s songs with HalaCG Radio!'
};