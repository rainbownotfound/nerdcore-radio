...
module.exports.run = (client, message, args) => {
    function infiniteLooper(dispatcherCB, connection) {
        dispatcherCB.on('end', () => {
            sql.get(`SELECT * FROM aliciamichelle ORDER BY RANDOM() LIMIT 1`).then(row => {
                ...
                sql.run(`CREATE TABLE IF NOT EXISTS aliciamichelle (link TEXT, name TEXT, creator TEXT, featuring TEXT)`).then(() => {
                    message.channel.send({
                        embed: utils.embed(`Error!`, `Alicia Michelle Radio is currently empty! Please contact the developers about this issue using \`${prefixes[message.guild.id].prefixes}reportbug\``)
                    });
                });
            });
        });
    ...
    if(!action) {
        if(!voiceChannel) {
            ...
            client.channels.get("491661236606468107").send({
                ...
                ${message.author.tag} just received an error for **AliciaMichelle Radio**!
            ...
            sql.get(`SELECT * FROM aliciamichelle ORDER BY RANDOM() LIMIT 1`).then(row1 => {
                sql.get(`SELECT * FROM playing WHERE guildID = "${message.guild.id}"`).then(row2 => {
                    if(!row2) {
                        sql.run(`INSERT INTO playing (guildID, isReady) VALUES (?, ?)`, [message.guild.id, "Yes"]);
                ...
                voiceChannel.join()
                    .then(connection => {
                        ...
                        client.channels.get("491651076156686336").send({
                            embed: utils.embed(`Radio Activated!`, `${message.author.tag} has activated **AliciaMichelle Radio** in ${message.guild.name} (${message.guild.id})`)
                        });
                    });
                    } else {
                        message.channel.send({
        ...
        if(message.author.id !== config.devs.rainbow && message.author.id !== config.devs.cernine && message.author.id !== config.devs.elliot) {
            ...
            if(!songLink) {
                ...
                sql.get(`SELECT * FROM aliciamichelle WHERE name = "${args[1]}"`).then(row => {
                    if(!row) {
                        sql.run(`INSERT INTO aliciamichelle (link, name, creator, featuring) VALUES (?, ?, ?, ?)`, [songLink, args[1], args[2], args[3]]);
        ...
        if(message.author.id !== config.devs.rainbow && message.author.id !== config.devs.cernine && message.author.id !== config.devs.elliot) {
            ...
            sql.get(`SELECT * FROM aliciamichelle WHERE name = "${args[1]}"`).then(row => {
                if(!row) {
                    ...
                    sql.run(`DELETE FROM aliciamichelle WHERE name = "${args[1]}"`);
                    message.channel.send({
...
module.exports.help = {
    command: 'aliciamichelle',
    usage: 'aliciamichelle',
    category: 'Radios',
    description: 'Listen to Alicia Michelle\'s songs with AliciaMichelle Radio!'
};