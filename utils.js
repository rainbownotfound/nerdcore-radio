...
const randomFooter = () => {
    return exports.randomSelection([
        'Welcome to Nerdcore Radio!',
        'Nerdcore Radio! The best nerdcore, brought to you on Discord!',
        'Currently supporting 12 YouTubers!',
        `Made by ${client.users.get(config.devs.rainbow).tag}, ${client.users.get(config.devs.elliot).tag} and ${client.users.get(config.devs.cernine).tag}`,
        'Too scared of the Support Server? You can always mail us at nerdcoreradio@rainbownotfound.org!'
    ]);
};

exports.randomTip = () => {
    return exports.randomSelection([
        'The bot is in Dan Bull\'s Discord server!',
        'There are many more YouTubers we want to support!',
        'You can sign up for updates! Use \`n/updates add\` to start!',
        'All radios use online databases to store the songs!',
        'You can suggest your favourite artist! Simply use \`n/suggest\` to start!',
        'You can unlock the Bundle Radio for 12 hours by upvoting the bot [here](https://discordbots.org/bot/477713081385091072/vote)!'
    ]);
}
...
exports.gistUpload = (text, lang = 'js') => {
    const filename = 'saitamabot_upload.' + lang;
